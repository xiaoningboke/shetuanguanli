...
<div class="page-content clearfix">
    ...
    <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
            ...
            <td class="td-manage">
              ...
              <?php if(($vo["state"] == 0) ): ?><a onClick="member_stop(this,<?php echo ($vo["id"]); ?>)"  href="javascript:;" title="停用"  class="btn btn-xs btn-success"><i class="icon-ok bigger-120"></i></a> 
            <?php elseif(($vo["state"] == 1)): ?>
             <a onClick="member_start(this,<?php echo ($vo["id"]); ?>)"  href="javascript:;" title="启用"  class="btn btn-xs"><i class="icon-ok bigger-120"></i></a> 
            <?php else: ?>
              <td class="td-status"><span class="label label-defaunt radius"></span></td><?php endif; ?>
            
            <a title="编辑" onclick="member_edit('<?php echo ($vo["department_name"]); ?>','<?php echo ($vo["state"]); ?>','<?php echo ($vo["id"]); ?>')" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
            <a title="删除" href="javascript:;"  onclick="member_del(this,<?php echo ($vo["id"]); ?>)" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i></a>
            </td>
...
<div class="add_menber" id="add_menber_style" style="display:none">
    ...
    <ul class=" page-content">
     <li><label class="label_name">系部名称：</label><span class="add_name"><input value="" name="department_name" type="text"  class="text_add"/></span><div class="prompt r_f"></div></li>
...
/*用户-添加*/
 $('#member_add').on('click', function(){
    layer.open({
     ...
     ////////////////
    $.ajax({
        url:"<?php echo U('Admin/Index/adddepartment');?>",//请求地址
        data:addDat,//发送的数据
        type:'POST',//请求的方式
...
function updateState(id,state){
  ...
  $.ajax({
        url:"<?php echo U('Admin/Index/updateDepartmentState');?>",//请求地址
        data:data,//发送的数据
        type:'POST',//请求的方式
        success:function (argument) {
          console.log(argument) ;
